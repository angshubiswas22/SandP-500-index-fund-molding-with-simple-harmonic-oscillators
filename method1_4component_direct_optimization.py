...
# Define 4-Component Harmonic Cosine Equation
def shm_4component_model(t, A1, w1, p1, A2, w2, p2, A3, w3, p3, A4, w4, p4, C):
...
def main():
    print("=" * 70)
    print(" METHOD 1: DIRECT OPTIMIZATION - 4-COMPONENT HARMONIC MODEL")
    print("=" * 70)
    ...
    # 5. FFT Frequency Initialization (Extract Top 4 Frequencies)
    fft_vals = np.fft.rfft(y_train_scaled)
    fft_freqs = np.fft.rfftfreq(len(y_train_scaled))
    non_zero_idx = np.where(fft_freqs > 0)[0]
    top_4_idx = non_zero_idx[np.argsort(np.abs(fft_vals[non_zero_idx]))[-4:][::-1]]
    init_omegas = 2 * np.pi * fft_freqs[top_4_idx]
    ...
    # Parameter Bounds (4 * [Amplitude, Frequency, Phase] + Offset)
    lower_bounds = [0.0, 1e-6, -np.pi] * 4 + [-5.0]
    upper_bounds = [10.0, np.pi, np.pi] * 4 + [5.0]
    bounds = (lower_bounds, upper_bounds)

    print("\nExecuting Non-Linear Curve Fitting (13 Parameters)...")
    popt, _ = curve_fit(
        shm_4component_model,
        t_train,
        y_train_scaled,
        p0=p0,
        bounds=bounds,
    ...
    pred_train_scaled = shm_4component_model(t_train, *popt)
    pred_test_scaled = shm_4component_model(t_test, *popt)
    ...
    # 7. Metrics
    print("\n" + "=" * 50)
    print(" EVALUATION METRICS (Method 1: 4-Component Model)")
    print("=" * 50)
    ...
    plt.plot(df['date'].iloc[:train_size], pred_train_raw, label='4-Component Model Fit (Train)', color='#2ca02c', linestyle='--')
    plt.plot(df['date'].iloc[train_size:], pred_test_raw, label='4-Component Model Forecast (Test)', color='#d62728', linestyle='--')
    plt.title('S&P 500 Price Forecast - Method 1: 4-Component curve_fit', fontsize=12, fontweight='bold')
    plt.ylabel('Price ($)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('method1_4component_results.png', dpi=300)
    print("\nPlot saved as 'method1_4component_results.png'.")
