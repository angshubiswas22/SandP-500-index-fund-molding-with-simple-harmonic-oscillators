...
# Define 5-Component Harmonic Cosine Equation
def shm_5component_model(t, A1, w1, p1, A2, w2, p2, A3, w3, p3, A4, w4, p4, A5, w5, p5, C):
    c1 = A1 * np.cos(w1 * t + p1)
    c2 = A2 * np.cos(w2 * t + p2)
    c3 = A3 * np.cos(w3 * t + p3)
    c4 = A4 * np.cos(w4 * t + p4)
    c5 = A5 * np.cos(w5 * t + p5)
    return c1 + c2 + c3 + c4 + c5 + C

def main():
    print("=" * 70)
    print(" METHOD 1: DIRECT OPTIMIZATION - 5-COMPONENT HARMONIC MODEL")
    print("=" * 70)
    ...
    # 5. FFT Frequency Initialization (Extract Top 5 Frequencies)
    fft_vals = np.fft.rfft(y_train_scaled)
    fft_freqs = np.fft.rfftfreq(len(y_train_scaled))
    non_zero_idx = np.where(fft_freqs > 0)[0]
    top_5_idx = non_zero_idx[np.argsort(np.abs(fft_vals[non_zero_idx]))[-5:][::-1]]
    init_omegas = 2 * np.pi * fft_freqs[top_5_idx]

    p0 = [
        1.0, init_omegas[0], 0.0,
        0.8, init_omegas[1], 0.0,
        0.5, init_omegas[2], 0.0,
        0.3, init_omegas[3], 0.0,
        0.1, init_omegas[4], 0.0,
        0.0
    ]

    # Parameter Bounds (5 * [Amplitude, Frequency, Phase] + Offset)
    lower_bounds = [0.0, 1e-6, -np.pi] * 5 + [-5.0]
    upper_bounds = [10.0, np.pi, np.pi] * 5 + [5.0]
    bounds = (lower_bounds, upper_bounds)

    print("\nExecuting Non-Linear Curve Fitting (16 Parameters)...")
    popt, _ = curve_fit(
        shm_5component_model,
        t_train,
        y_train_scaled,
        p0=p0,
        bounds=bounds,
    ...
    pred_train_scaled = shm_5component_model(t_train, *popt)
    pred_test_scaled = shm_5component_model(t_test, *popt)
    ...
    # 7. Metrics
    print("\n" + "=" * 50)
    print(" EVALUATION METRICS (Method 1: 5-Component Model)")
    print("=" * 50)
    ...
    plt.plot(df['date'].iloc[:train_size], pred_train_raw, label='5-Component Model Fit (Train)', color='#2ca02c', linestyle='--')
    plt.plot(df['date'].iloc[train_size:], pred_test_raw, label='5-Component Model Forecast (Test)', color='#d62728', linestyle='--')
    plt.title('S&P 500 Price Forecast - Method 1: 5-Component curve_fit', fontsize=12, fontweight='bold')
    plt.ylabel('Price ($)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('method1_5component_results.png', dpi=300)
    print("\nPlot saved as 'method1_5component_results.png'.")
