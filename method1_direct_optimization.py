...
# Define 3-Component Harmonic Cosine Equation
def shm_3component_model(t, A1, w1, p1, A2, w2, p2, A3, w3, p3, C):
...
def main():
    print("=" * 70)
    print(" METHOD 1: DIRECT NON-LINEAR OPTIMIZATION (scipy.optimize.curve_fit)")
    print("=" * 70)
    ...
    # 2. Chronological 75:25 Train-Test Split
    ...
    # 5. FFT Frequency Initialization
    fft_vals = np.fft.rfft(y_train_scaled)
    fft_freqs = np.fft.rfftfreq(len(y_train_scaled))
    non_zero_idx = np.where(fft_freqs > 0)[0]
    top_3_idx = non_zero_idx[np.argsort(np.abs(fft_vals[non_zero_idx]))[-3:][::-1]]
    init_omegas = 2 * np.pi * fft_freqs[top_3_idx]

    p0 = [
        1.0, init_omegas[0], 0.0,
        0.5, init_omegas[1], 0.0,
        0.2, init_omegas[2], 0.0,
        0.0
    ]
    bounds = (
        [0, 1e-6, -np.pi, 0, 1e-6, -np.pi, 0, 1e-6, -np.pi, -5.0],
        [10.0, 0.5, np.pi, 10.0, 0.5, np.pi, 10.0, 0.5, np.pi, 5.0]
    )

    print("\nExecuting Non-Linear Curve Fitting...")
    popt, _ = curve_fit(shm_3component_model, t_train, y_train_scaled, p0=p0, bounds=bounds, maxfev=50000)

    # 6. Predict & Invert Transformations
    pred_train_scaled = shm_3component_model(t_train, *popt)
    pred_test_scaled = shm_3component_model(t_test, *popt)
    ...
    # 7. Metrics
    print("\n" + "=" * 50)
    print(" EVALUATION METRICS (Method 1)")
    print("=" * 50)
    ...
    plt.plot(df['date'].iloc[:train_size], pred_train_raw, label='Model Fit (Train)', color='#2ca02c', linestyle='--')
    plt.plot(df['date'].iloc[train_size:], pred_test_raw, label='Model Forecast (Test)', color='#d62728', linestyle='--')
    plt.title('S&P 500 Price Forecast - Method 1: curve_fit', fontsize=12, fontweight='bold')
    plt.ylabel('Price ($)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('method1_curve_fit_results.png', dpi=300)
    print("\nPlot saved as 'method1_curve_fit_results.png'.")
